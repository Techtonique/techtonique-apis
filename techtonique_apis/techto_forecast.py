...
@arg(
    "df",
    index=False,
    doc="Excel range with columns 'date' and one or more series.",
)
...
@arg(
    "return_sims",
    doc="If TRUE, return simulation matrix; else, return forecast summary (default: FALSE)",
)
@arg(
    "series_choice",
    doc="(Optional) Name of the series to forecast if multiple are present",
)
    ...
    if df.shape[1] == 2:  # univariate time series
        ...
        if series_choice is not None and series_choice in series_names:
            ...
            if return_sims:
                ...
                sims_selected = [
                    [h[idx] for h in rep] for rep in sims
                ]  # shape: (replications, horizon)
                sims_df = pd.DataFrame(sims_selected).transpose()
                sims_df.columns = [
                    f"sim_{i+1}_{series_choice}"
        ...
        for stat in ["mean", "lower", "upper"]:
            for s in range(n_series):
                summary_data[f"{stat}_{series_names[s]}"] = [
                    x[s] for x in result[stat]
                ]
